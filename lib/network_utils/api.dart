...
class Network {
  final String _baseUrl = 'https://2bce3eaa4bc1.ngrok.io/api';
  var token;
  ...
  Future<http.Response> postWithoutAuth(Map <String, String> data, String apiUrl) {
  ...
  Future<http.Response> registerUser(User user, File avatar, String password, List<Drug> drugs, String apiUrl) async {
    var url = _baseUrl + apiUrl;
    ...
    print(request.fields['drugs'] );

    if (avatar != null) {
      var stream = http.ByteStream(DelegatingStream.typed(avatar.openRead()));
      var fileLenght = await avatar.length();
      var multipartFileSign = http.MultipartFile('avatar', stream, fileLenght, filename: basename(avatar.path));
  ...
  Future<http.Response> putWithAuth(Map <String, dynamic> data, String apiUrl) async{
    var url = _baseUrl + apiUrl;
    print(url);
    await _getToken();
    token = token.replaceAll("\"", "");
